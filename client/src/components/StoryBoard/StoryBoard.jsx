...
const StoryBoard = (props) => {
    ...
    const displayList = () => {
        ...
        story.forEach((slide, index) => {
            if (index === storyLeft) {
                display.push(
                    <div className="story__row" key={uuid()}>
                        <div className="story__check"></div>
                        <h3
                            className="story__stage--current"
                            onClick={() => change('playing')}
                        >
                            {slide.title}
                        </h3>
                    </div>
                ...
                display.push(
                    <div className="story__row" key={uuid()}>
                        <div className="story__check"></div>
                        <h3 className="story__stage--blur">{slide.title}</h3>
                    </div>
                );
            } else {
                display.push(
                    <div className="story__row" key={uuid()}>
                        <div className="story__check">
                            <img
                                src={check}
                        ...
                        <h3 className="story__stage">{slide.title}</h3>
                    </div>
                );
            }
        });
    ...
    return (
        <div>
            {displayBoard && (
                <div className="story" onClick={show}>
                    <div className="story__wrap" onClick={stopPropagation}>
                        <img
                            src={clip}
                            className="story__board"
                            alt="Cartoon clipboard"
                        />
            ...
            {!displayBoard && <ToggleButton display={show} />}
        </div>
    );
};
